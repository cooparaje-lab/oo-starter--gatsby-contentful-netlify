...
const CardRecursos = ({ card }) => (
  <div className="relative flex flex-col w-full mb-2 overflow-hidden bg-gray-800 rounded shadow-lg h-60 from-gray-800 via-gray-800 bg-gradient-to-br lg:h-64 lg:flex-row">
    <div className="absolute inset-0 z-0 block w-full h-full group opacity-5 lg:opacity-90 lg:relative lg:w-full lg:h-64">
    ...
    <div className="z-10 flex flex-col items-center justify-center w-full px-6 py-2 pb-6 lg:pb-6">
      <div className="flex items-baseline justify-between w-full mt-2 mb-1">
        <Link
          to={`/recursos/${card.slug}`}
          className="block w-full mb-2 font-serif text-3xl font-bold text-center text-yellow-500 capitalize hover:underline hover:text-yellow-300 "
        >
          {card.title}
        </Link>
      </div>
      <p className="mb-1 font-sans text-xl text-center text-gray-100 line-clamp-3">
        {card.excerpt.childMarkdownRemark.excerpt}
      </p>

      {card.espacio && (
        <div className="z-50 flex items-center justify-center w-full mt-2 space-x-2">
          {card.espacio.map((espacio, i) => [
            <Link
              ...
              key={i}
            >
              <span className="inline-block w-6 text-xs text-center lg:mr-2 lg:text-xl ">
                {espacio.icono}
              </span>
              <span className="font-mono text-xs font-bold uppercase lg:text-sm hover:text-gray-500">
                {espacio.title}
              </span>
            </Link>,
          ])}
      ...
      <a
        className="absolute bottom-0 right-0 z-20 flex items-center justify-center p-2 m-2 font-mono text-2xl font-bold text-white transition-all duration-200 bg-gray-700 rounded hover:bg-green-700"
        href={card.url}
        target="_blank"
